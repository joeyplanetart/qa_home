...
| 模块 | 说明 |
|------|------|
| 📦 项目 | 24+ 站点按业务线分组，展示域名、环境、内容统计、**自动化用例数**与健康状态 |
...
| ✅ 每日清单 | 可勾选的任务列表，支持重置 |
| 🤖 UI 自动化 | Playwright + pytest 本地执行，用例管理、运行配置、报告与测试数据 |
...
./scripts/install-playwright.sh # UI 自动化所需（可选）
./scripts/run.sh                # 启动 → http://localhost:8765
```

访问 **http://localhost:8765/automation** 进入 UI 自动化管理页。
...
```
qa_home/
├── automation/             # UI 自动化测试（Playwright + pytest）
│   ├── conftest.py         # 公共 fixture、截图、测试数据
│   ├── pytest.ini
│   ├── test_data.py        # 运行时测试数据（账号、订单等）
│   └── suites/             # 按站点/项目分套件
│       └── cafepress/      # 示例：CafePress E2E
│           ├── meta.json   # 套件元数据（name、projectId）
│           ├── pages/      # Page Object
│           └── test_*.py
├── server/
│   ├── app.py              # FastAPI 路由与 API
│   ├── db.py               # 数据库连接（Turso / SQLite）
│   ├── runner.py           # UI 自动化执行引擎
...
│   ├── stop.sh             # 停止服务
│   ├── kill-port.sh        # 释放占用端口
│   └── install-playwright.sh  # 安装 Playwright 浏览器
├── docs/                   # 文档
├── data/                   # 本地 SQLite（git 忽略）
├── reports/                # 自动化测试报告与产物（git 忽略）
├── run.py                  # 开发入口
├── requirements.txt
├── pyproject.toml
...
./scripts/install-playwright.sh  # 安装 Playwright Chromium（UI 自动化）
```
